...
def main():      
    try:          
        ...
        #mo_key='202210'
        mo_key=sys.argv[1]   
        output_dir='/DATALAKE_TLS_TEST/EXP_CHURN_POSTPAID_DATA/' + mo_key      
        # Init spark session 
        sparkSession = SparkSession.builder.appName("EXP_CHURN_POSTPAID_DATA").config("spark.sql.crossJoin.enabled", "true").enableHiveSupport().getOrCreate();
        
        # Step - 1
        # Xac dinh tap thue bao CHURN_IND = 1(Thue bao tra sau roi mang)
        dfChurnInd1 = sparkSession.sql("""  SELECT                                                 
                                            ...
                                            FROM MBF_BIGDATA.ADMD_CHURN_KPI_MO A
                                            LEFT JOIN MBF_BIGDATA.ADMR_ACCOUNT_SERVICE B
                                            ON A.SERVICE_NBR = B.SERVICE_NBR AND A.ACCT_SRVC_INSTANCE_KEY = B.ACCT_SRVC_INSTANCE_KEY AND A.ACCT_KEY = B.ACCT_KEY AND A.PROD_LINE_KEY = B.PROD_LINE_KEY 
                                            AND B.DAY_KEY = DATE_FORMAT(LAST_DAY(FROM_UNIXTIME(UNIX_TIMESTAMP('""" + mo_key + """', 'yyyyMM'))),'yyyyMMdd')
                                            WHERE A.PROD_LINE_KEY = 2 AND CHURN_IND = 1 
                                            AND A.LFCYCL_STAT_CD NOT IN('3','4') AND A.PROD_LINE_KEY IS NOT NULL AND A.NTWK_QOS_GRP_CD NOT IN('FC') AND A.CUST_NAME IS NOT NULL
                                            AND MO_KEY = """ + mo_key)
        churnInd1Cnt = dfChurnInd1.count()
        print('Tong so thue bao tra sau roi mang: ' + str(churnInd1Cnt))

        # Xac dinh tap thue bao CHURN_IND = 0
        dfChurnInd0 = sparkSession.sql("""  SELECT 
                                                A.SERVICE_NBR                                                                               AS SERVICE_NBR,                                                 
                                                A.ACCT_SRVC_INSTANCE_KEY                                                                    AS ACCT_SRVC_INSTANCE_KEY,
                                                A.ACCT_KEY                                                                                  AS ACCT_KEY,
                                                A.CUST_KEY                                                                                  AS CUST_KEY,
                                            ...
                                            ON A.SERVICE_NBR = B.SERVICE_NBR AND A.ACCT_SRVC_INSTANCE_KEY = B.ACCT_SRVC_INSTANCE_KEY AND A.ACCT_KEY = B.ACCT_KEY AND A.PROD_LINE_KEY = B.PROD_LINE_KEY 
                                            AND B.DAY_KEY = DATE_FORMAT(LAST_DAY(FROM_UNIXTIME(UNIX_TIMESTAMP('""" + mo_key + """', 'yyyyMM'))),'yyyyMMdd')
                                            WHERE A.PROD_LINE_KEY = 2 AND CHURN_IND = 0 
                                            AND A.LFCYCL_STAT_CD NOT IN('3','4') AND A.PROD_LINE_KEY IS NOT NULL AND A.NTWK_QOS_GRP_CD NOT IN('FC') AND A.CUST_NAME IS NOT NULL
                                            AND MO_KEY = """ + mo_key + """ LIMIT """ + str(churnInd1Cnt*3))
        churnInd0Cnt = dfChurnInd0.count()        
        
        df = dfChurnInd1.unionAll(dfChurnInd0)
        print('Tong so mau dua vao mining: ' + str(df.count()))
        if df.count()>0:
            df.coalesce(10).write.mode("overwrite").option('header','true').csv(output_dir)        
